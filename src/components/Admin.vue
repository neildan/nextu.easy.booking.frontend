...
export default {
  data() {
    return {
      ...
      errorEventImg: {
        show: false,
        message: "Subir una imagen con tamaño maximo de 5M es obligatorio",
      },
      errorTheaterName: {
        show: false,
  ...
  methods: {
    ...
    async actionsForm() {
      ...
      try {
        switch (this.statusFormModal) {
          case "theater":
            ...
            route =
              this.idRoute == 0
                ? routeTheaterHalls
                : `${routeTheaterHalls}/${this.idRoute}`;
            params = {
            ...
            break;
          case "event":
            let foto = document.getElementById("imgFunction").files[0];
            if (!this.validateEvent(foto)) throw false;
            route =
              this.idRoute == 0
                ? routeEvents
                : `${routeEvents}/${this.idRoute}`;
            this.formEvent.url_image = `img-${Math.floor(
              Math.random() + 1 * 10
            )}.jpg`;
            params = {
              method: this.idRoute == 0 ? "POST" : "PUT",
          ...
          case "function":
            ...
            break;
          case "d-theater":
            route = `${routeTheaterHalls}/${this.idRoute}`;
            params = {
              method: "DELETE",
            };
            break;
          case "d-event":
            route = `${routeEvents}/${this.idRoute}`;
            params = {
              method: "DELETE",
            };
            break;
          case "d-function":
            route = `${routeFunctions}/${this.idRoute}`;
            params = {
              method: "DELETE",
            };
            break;
        ...
        try {
          ...
          this.messageModalSuccessForm = "¡La petición fue exitosa!";
    ...
    validateEvent(img) {
      try {
        ...
        if (!img || img.size > 500000) {
          this.errorEventImg.show = true;
          throw false;
        } else {
          this.errorEventImg.show = false;
        }
        return true;
      } catch (e) {
        this.messageModalErrorForm =
    ...
    async checkUserAdmin() {
      try {
        const response = await fetch(`${host}${routeAdmin}`, {
          method: "POST",
          body: JSON.stringify({
        ...
        const dataResponse = await response.json();
        if (!dataResponse) throw "Fallo al intentar conectarse al Backend";
        if (!dataResponse.success) throw dataResponse.msj;
...
<template>
    ...
    <div class="row">
      ...
      <div class="col-md-9 col-sm-12">
        <div v-show="moduleTheaterHalls">
          ...
          <p class="float-start">
            Los teatros son donde nace todo, son el lugar físico donde ocurre
            los eventos
          </p>
          <button
            type="button"
          ...
          <p v-if="dataTheaters.length == 0" class="float-start mt-4 col-12">
        ...
        <div v-show="moduleEvents">
          ...
          <p v-if="dataEvents.length == 0" class="float-start mt-4 col-12">
            No hay registros disponibles
          </p>
          <div v-else class="mt-5">
            <div class="table-responsive col-12 mb-5">
        ...
        <div v-show="moduleFunctions">
          ...
          <p class="float-start">
            Las funciones son la cantidad de veces que se va a repetir un evento
            y en qué horario
          </p>
          <button
            type="button"
          ...
          <p v-if="dataFunctions.length == 0" class="float-start mt-4 col-12">
            No hay registros disponibles
          </p>
          <div v-else class="mt-5">
            <div class="table-responsive col-12 mb-5">
                      ...
                      >
                        <ol>
                          <li
                            ...
                            :key="spectatorF.id"
                          >
                            {{ getSpectator(spectatorF.spectator_id) }}
                          </li>
                        </ol>
                      </span>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div v-show="isEditCreateEvent">
              ...
              <div class="mb-3">
                <label for="imgFunction" class="form-label"
                  >Suba una foto</label
                >
                <input
                  class="form-control"
                  ...
                  id="imgFunction"
                />
                <small class="error" v-show="errorEventImg.show">{{
                  errorEventImg.message
                }}</small>
              </div>
            </div>
            ...
            <p class="text-center success">{{ messageModalSuccessForm }}</p>
          </div>
  ...
  <section v-else>
    <div class="col-12">
      <h1 class="text-center">No tiene acceso a este módulo</h1>
    </div>
  </section>
</template>
